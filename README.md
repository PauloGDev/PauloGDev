<!-- ===================================================== -->
<!--                   PAULO OLIVEIRA                      -->
<!-- ===================================================== -->

<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=320&color=0:0D0221,25:240046,50:5A189A,75:00F5D4,100:00BBF9&text=PAULO%20OLIVEIRA&fontSize=62&fontColor=ffffff&animation=fadeIn&fontAlignY=36&desc=JAVA%20BACKEND%20%20•%20%20SPRING%20BOOT%20%20•%20%20FULLSTACK%20ENGINEER&descSize=18&descAlignY=58"/>

<br>

<img src="https://readme-typing-svg.herokuapp.com?font=Orbitron&weight=700&size=30&duration=2500&pause=900&center=true&vCenter=true&width=900&height=70&color=00F5D4&background=00000000&lines=Java+Backend+Developer;Spring+Boot+Engineer;REST+API+Architect;React+%26+Next.js+Developer;Building+Scalable+Software;Always+Learning"/>

<br><br>

<img src="https://img.shields.io/badge/STATUS-AVAILABLE_FOR_WORK-00F5D4?style=for-the-badge&labelColor=0D0221"/>

<img src="https://img.shields.io/badge/LOCATION-Brazil-7B2CBF?style=for-the-badge"/>

<img src="https://img.shields.io/badge/OPEN_TO_REMOTE-YES-00C853?style=for-the-badge"/>

<br><br>

<a href="mailto:paulovops@outlook.com">

<img src="https://img.shields.io/badge/EMAIL-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>

</a>

<a href="https://www.linkedin.com/in/paulo-oliveira-9932912b5/">

<img src="https://img.shields.io/badge/LINKEDIN-0077B5?style=for-the-badge&logo=linkedin"/>

</a>

<a href="https://github.com/PauloGDev">

<img src="https://img.shields.io/badge/GITHUB-181717?style=for-the-badge&logo=github"/>

</a>

<br><br>

<img src="https://komarev.com/ghpvc/?username=PauloGDev&label=PROFILE+VIEWS&color=00F5D4&style=for-the-badge"/>

</div>

---

<div align="center">

```text
╔══════════════════════════════════════════════════════════════╗
║                                                             ║
║      JAVA • SPRING • REACT • POSTGRESQL • DOCKER           ║
║                                                             ║
║      BUILDING SCALABLE SOFTWARE FOR THE FUTURE             ║
║                                                             ║
╚══════════════════════════════════════════════════════════════╝
```

</div>

---

# 👋 About Me

```java
public class PauloOliveira {

    String role = "Full Stack Software Developer";

    String[] specialization = {
        "Java",
        "Spring Boot",
        "Backend Development",
        "React",
        "Software Engineering"
    };

    String[] currentlyLearning = {
        "Microservices",
        "Docker",
        "AWS",
        "CI/CD",
        "Software Architecture"
    };

    String[] interests = {
        "Backend Development",
        "Scalable Systems",
        "Cloud Computing",
        "Clean Architecture",
        "REST APIs"
    };
}
```

I'm a **Full Stack Software Developer** passionate about building scalable, secure and modern applications.

My primary focus is **Java Backend Development**, designing robust REST APIs with **Spring Boot**, while also creating responsive frontend applications using **React** and **Next.js**.

I enjoy turning ideas into complete products, from database modeling to deployment in production environments.

I have developed complete systems including:

- 🍽 Restaurant SaaS
- 🎓 Student Management System
- 🚗 Dealership Management System
- 🛒 E-commerce Platform

My goal is to become an outstanding Software Engineer by continuously learning and building impactful software.

---

# 🚀 Tech Stack

## Languages

<p align="center">

<img src="https://skillicons.dev/icons?i=java,js,html,css"/>

</p>

---

## Backend

<p align="center">

<img src="https://skillicons.dev/icons?i=spring,maven"/>

</p>

<p align="center">

<img src="https://img.shields.io/badge/Spring%20Security-6DB33F?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Hibernate-59666C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/JPA-59666C?style=for-the-badge"/>
<img src="https://img.shields.io/badge/JWT-black?style=for-the-badge"/>
<img src="https://img.shields.io/badge/REST%20API-blue?style=for-the-badge"/>

</p>

---

## Frontend

<p align="center">

<img src="https://skillicons.dev/icons?i=react,nextjs"/>

</p>

---

## Database

<p align="center">

<img src="https://skillicons.dev/icons?i=postgres,mysql"/>

</p>

---

## DevOps

<p align="center">

<img src="https://skillicons.dev/icons?i=docker,git,github,linux"/>

</p>

<p align="center">

<img src="https://img.shields.io/badge/Railway-000000?style=for-the-badge"/>
<img src="https://img.shields.io/badge/NGINX-009639?style=for-the-badge"/>

</p>

---

## Software Engineering

<p align="center">

<img src="https://img.shields.io/badge/Clean%20Code-222222?style=for-the-badge"/>

<img src="https://img.shields.io/badge/SOLID-00599C?style=for-the-badge"/>

<img src="https://img.shields.io/badge/Clean%20Architecture-4CAF50?style=for-the-badge"/>

<img src="https://img.shields.io/badge/OOP-FF9800?style=for-the-badge"/>

<img src="https://img.shields.io/badge/Scrum-6DB33F?style=for-the-badge"/>

<img src="https://img.shields.io/badge/Kanban-1976D2?style=for-the-badge"/>

</p>

---

# 📊 GitHub Statistics

<div align="center">

<img height="180em" src="https://github-readme-stats.vercel.app/api?username=PauloGDev&show_icons=true&theme=tokyonight&hide_border=true&rank_icon=github"/>

<img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=PauloGDev&layout=compact&theme=tokyonight&hide_border=true"/>

</div>

<br>

<div align="center">

<img src="https://streak-stats.demolab.com?user=PauloGDev&theme=tokyonight&hide_border=true"/>

</div>

---

# 📈 Contribution Graph

<div align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=PauloGDev&theme=tokyo-night&hide_border=true"/>

</div>

---

# 🏆 GitHub Trophies

<div align="center">

<img src="https://github-profile-trophy.vercel.app/?username=PauloGDev&theme=tokyonight&no-frame=true&no-bg=true&margin-w=15&column=4"/>

</div>

---

# 🚀 Featured Projects

<div align="center">

> Real-world applications built using modern software engineering practices.

</div>

---

# 🍽 Restaurant SaaS

> **Complete SaaS platform for restaurant management.**

A modern web application designed to streamline restaurant operations, offering a complete management ecosystem for owners, employees and customers.

### ✨ Features

- 🔐 Secure Authentication & Authorization
- 🍔 Digital Menu Management
- 🪑 Table Reservation & Control
- 🧾 Order Management
- 👨‍🍳 Kitchen Dashboard
- 📦 Inventory Management
- 👥 Customer Management
- 📈 Administrative Dashboard
- 📊 Sales Reports
- 🌐 Responsive Interface
- 🔄 REST API Integration

### 🛠 Technologies

<p>

<img src="https://skillicons.dev/icons?i=java,spring,react,postgres,docker"/>

</p>

Spring Security • Hibernate • JWT • Maven • Railway • Git • GitHub

### 🔗 Repository

👉 https://github.com/PauloGDev/DigitalTricks-Restaurant-OpenSource

---

# 🎓 Student Management System

> **Complete academic management platform built with Java and React.**

A full-stack application developed to simplify educational administration, providing secure access, academic management and a responsive interface.

### ✨ Features

- 👨‍🎓 Student Management
- 👨‍🏫 Teacher Management
- 📚 Course Management
- 📝 Enrollment System
- 🔐 JWT Authentication
- 👥 Role-Based Access Control
- 📊 Dashboard
- 📑 Reports
- 📄 CRUD Operations
- 🌐 REST APIs
- 📱 Responsive Frontend
- ⚠ Exception Handling
- ✅ Data Validation
- 📖 Swagger Documentation

### 🛠 Technologies

<p>

<img src="https://skillicons.dev/icons?i=java,spring,react,postgres,docker"/>

</p>

Spring Security • Hibernate • JPA • JWT • Maven • Swagger • Railway

---

# 🚗 Dealership Management System

> **Complete vehicle dealership management platform.**

Designed to simplify dealership operations through customer, vehicle and sales management.

### ✨ Features

- 🚗 Vehicle Inventory
- 👥 Customer Registration
- 💰 Sales Management
- 📈 Reports
- 🔐 Authentication
- 📊 Dashboard
- 🔄 REST APIs
- 📱 Responsive Interface

### 🛠 Technologies

<p>

<img src="https://skillicons.dev/icons?i=java,spring,react,postgres"/>

</p>

Spring Security • JWT • Hibernate • Maven • Git

### 🔗 Repository

👉 https://github.com/PauloGDev/DigitalTricks-Concessionarias

---

# 🛒 E-Commerce Platform

> **Complete e-commerce platform with secure authentication and administrative dashboard.**

A modern shopping platform implementing essential e-commerce features following clean architecture principles.

### ✨ Features

- 🛍 Product Catalog
- 🛒 Shopping Cart
- 💳 Checkout
- 📦 Order Management
- 🔐 Authentication
- 👤 Customer Area
- 📊 Administrative Dashboard
- 💰 Payment Integration
- 📱 Responsive Design
- 🌐 REST APIs

### 🛠 Technologies

<p>

<img src="https://skillicons.dev/icons?i=java,spring,react,postgres,docker"/>

</p>

Spring Security • Hibernate • JWT • Git • Railway

### 🔗 Repository

👉 https://github.com/PauloGDev/sistema-ecommerce-completo

---

# 💼 Professional Experience

### 💻 Full Stack Developer

**Java • Spring Boot • React • PostgreSQL**

Responsible for designing, developing and maintaining complete web applications, integrating backend and frontend technologies while following software engineering best practices.

### Main Responsibilities

- ⚙ Backend Development with Java & Spring Boot
- 🔐 Authentication using Spring Security & JWT
- 🌐 RESTful API Design
- 🗄 Database Modeling
- 📱 Frontend Development with React
- 🐳 Docker Containerization
- ☁ Cloud Deployment
- 🔄 Git Version Control
- 📚 Technical Documentation
- 🧪 Functional Testing

---

# 📚 Software Engineering Principles

✔ Clean Code

✔ SOLID Principles

✔ Layered Architecture

✔ Object-Oriented Programming

✔ RESTful API Design

✔ Database Modeling

✔ Agile Development

✔ Scrum

✔ Kanban

✔ Responsive Design

✔ Software Documentation

✔ Functional Testing

---

# 🌱 Currently Learning

I'm constantly improving my skills in:

- ☁ AWS
- ☸ Kubernetes
- 🐳 Advanced Docker
- 🔗 Microservices
- 🚀 CI/CD
- 🏛 Software Architecture
- 📦 Apache Kafka
- ⚡ Redis
- 🔥 Spring Cloud

---

---

# 🎯 Career Goals

```text
🎯 Become a Senior Java Backend Engineer

🏗 Design scalable and high-performance systems

☁ Master Cloud Computing (AWS)

🔗 Build distributed Microservices

🚀 Contribute to international software teams

📚 Continuously improve as a Software Engineer
```

---

# 💡 Fun Facts

```java
public class Developer {

    String coffee = "Always ☕";

    String favoriteStack =
        "Java + Spring Boot + React";

    String philosophy =
        "Keep Learning Every Day";

    String mission =
        "Build software that makes people's lives easier.";
}
```

---

# 🌎 Connect With Me

<div align="center">

<a href="mailto:paulovops@outlook.com">

<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>

</a>

<a href="https://www.linkedin.com/in/paulo-oliveira-9932912b5/">

<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>

</a>

<a href="https://github.com/PauloGDev">

<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>

</a>

</div>

---

# 📫 Contact

📧 **Email**

```text
paulovops@outlook.com
```

💼 **LinkedIn**

```text
linkedin.com/in/paulo-oliveira-9932912b5
```

🐙 **GitHub**

```text
github.com/PauloGDev
```

📍 **Location**

```text
Fortaleza - Ceará - Brazil
```

---

# 💻 Development Philosophy

> "Good software is not just code that works.  
> It is code that is clean, maintainable, scalable and built to solve real problems."

---

# 📊 Weekly Development Focus

```
Backend Development      ████████████████████ 100%

Spring Boot              ███████████████████░ 95%

React                    █████████████████░░░ 85%

Database Design          ██████████████████░░ 90%

Software Architecture    ███████████████░░░░░ 75%

Cloud Computing          ██████████░░░░░░░░░░ 50%
```

---

# ❤️ Open Source

I'm always interested in:

- Contributing to Open Source
- Collaborating on Java projects
- Learning new technologies
- Sharing knowledge
- Building impactful software

Feel free to open an Issue or Pull Request!

---

# 🐍 Contribution Snake

<div align="center">

> **Configure this GitHub Action to enable the animation**

<img src="https://raw.githubusercontent.com/Platane/snk/output/github-contribution-grid-snake-dark.svg"/>

</div>

---

# ⭐ Quote

<div align="center">

> **"First, solve the problem. Then, write the code."**

**— John Johnson**

</div>

---

# 🚀 Thanks for Visiting!

<div align="center">

### Thank you for visiting my profile!

I'm always open to discussing technology, software engineering and exciting opportunities.

If you enjoyed my projects, consider giving them a ⭐.

Let's build something amazing together!

<br>

<img src="https://capsule-render.vercel.app/api?type=waving&color=0:0F2027,50:203A43,100:2C5364&height=140&section=footer"/>

</div>
